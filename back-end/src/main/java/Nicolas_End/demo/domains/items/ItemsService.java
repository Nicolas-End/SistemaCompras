...
import Nicolas_End.demo.dtos.items.*;
...
public class ItemsService {
    ...
    final private ItemsRepository itensRepository;
    final private ResponseUtil responseUtil ;
    final private ProviderService providerService;

    public ItemsService(ItemsRepository itensRepository, ResponseUtil responseUtil, ProviderService providerService){
    ...
    public  <T> ApiResponse registerNewItem(ItemAndProviderCnpjDTO item){
        ...
        ItemsEntity itemEntityRegistered = this.itensRepository.save(itemEntity);
    ...
    public List<ItemEntityAndQuantityDTO> getManyItemByItemQuantityDTO(List<ItemQuantityDTO> items){

        // extraindo todos os id do DTO
        List<UUID> ids = items.stream()
                // .map(item -> item.id())
                .map(ItemQuantityDTO::id)
                .toList();

        // "juntando" as entidades e ids em uma lista unica
        Map<UUID, ItemsEntity> entityMap = this.itensRepository.findAllById(ids)
                .stream()
                .collect(Collectors.toMap(ItemsEntity::getId, Function.identity()));

        return items.stream()
                .map(item -> {
                    ItemsEntity entity = entityMap.get(item.id());
                    if (entity == null) throw new EntityNotFoundException("Item não encontrado: " + item.id());
                    return new ItemEntityAndQuantityDTO(entity, item.quantity());
                })
                .toList();
